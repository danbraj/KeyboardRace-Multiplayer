# "Wyścig na klawiaturze" v0.9

## Temat projektu

Aplikacja do ćwiczenia szybkiego pisania na klawiaturze z elementami rywalizacji. W grze może uczestniczyć do sześciu graczy. Zadaniem użytkowników jest przepisanie tekstu najszybciej ze wszystkich. Serwer wyznacza w sposób losowy zadanie czyli tekst do przepisania. W trakcie zabawy gracze dostają "punkty akcji", które mogą wykorzystać na umiejętności specjalne (ukrycie wprowadzanych liter, pomieszanie liter w przepisywanym tekście, odwrócenie wyrazów w tekście), by przeszkodzić konkurentom . Dodatkowo, każdy użytkownik (klient) może wpłynąć na rozwój aplikacji, dodając ciekawe teksty do przepisania. Jednak dodany tekst pojawi się w aplikacji, dopiero po potwierdzeniu ze strony serwera.

## Konfiguracja aplikacji

### Opis klas

* Consts - Klasa zawierająca stałe statyczne wykorzystywane w aplikacjach klienta oraz serwera. Pełni rolę konfiguracji. W klasie tej znajduje się jeszcze pseudo- typ wyliczeniowy pełniący funkcję flagi. Konfiguracji podlega:
  * zmiana wersji aplikacji,
  * ustalenie maksymalnej liczby użytkowników, którzy mogą się zalogować do serwera,
  * ustalenie maksymalnej liczby próśb z nowymi zadaniami, które serwer może zmagazynować (ochrona przed spamem),
  * ustalenie czy w ogóle jest dostępna możliwość wysyłania próśb.
* FilesService - Klasa statyczna pełniąca funkcję obsługi plików w aplikacji serwera. Za pomocą tej klasy można pobrać wszystkie pliki tekstowe znajdujące się w ustalonej w kodzie lokalizacji lub zapisać przesłany w parametrze tekst w pliku.
...
### Opis sposobu komunikacji klient-serwer

Aplikacja, do komunikacji pomiędzy klientami a serwerem, wykorzystuje serializację obiektów (klasa Packet i ExtendedPacket) zawierących stosowne właściwości, zależne od właściwości protokołu (typ wyliczeniowy Command).
...
### Schemat klas używanych do komunikacji klient-serwer
...
### Opis interfejsu graficznego aplikacji serwera

Interfejs graficzny serwera składa się z 4 istotnych elementów:
* Przełącznik “Uruchom/Zatrzymaj” - służy do uruchamiania i zatrzymywania serwera.
* Pole tekstowe oznaczone etykietą “Port:” - służy do ustalenia, na którym porcie uruchomiony zostanie serwer.
* Obszar tekstowy - informuje o różnych zdarzeniach serwera
* Przycisk “Pokaż odebrane zadania (x)” - wyświetla, wysłane przez klienta, zadanie w wyskakującym oknie, gdzie może zostać zaakceptowane w niezmienionej formie, bądź poprawione lub całkiem odrzucone przez administratora serwera.
...
* SerwerGUI - Główna klasa serwera. Deklaruje wszystkie parametry GUI oraz tworzy graficzny interfejs aplikacji serwera. Klasa ta zajmuje się także losowaniem zadań (plików z zadaniem w folderze Texts, z których tworzony jest obiekt klasy Zadanie) do wykonania.
...
* Server - Klasa rozszerzająca klasę Thread. Uruchomienie serwera wiąże się z utworzeniem obiektu tej klasy oraz uruchomieniem go jako wątek. Zatrzymanie serwera to zakończenie działania wątku serwera. Operacji uruchomienia, bądź zatrzymania serwera towarzyszy także zmiana ustawień odpowiednich komponentów w klasie ServerGUI.
* Connection - Klasa rozszerzająca klasę Thread. Obiekt tej klasy jest tworzony po pomyślnym połączeniu się do serwera. Po utworzeniu obiektu uruchamiany jest jako wątek, gdzie przechodzi w stan nasłuchu na obiekty klasy Packet lub dziedziczące klasę Packet (ExtendedPacket) od klienta. Po otrzymaniu zserializowanego obiektu żądanego typu, wykonuje różne akcje zależne od protokołu oraz parametrów otrzymywanych od klienta.

### Diagram UML aplikacji serwera

...

## Klient

### Opis interfejsu graficznego aplikacji klienta

Najważniejszym elementem aplikacji jest szary obszar tekstowy. Jest on elementem “centralnym”, znaczy to tyle, że przy zmianie wielkości okna jest responsywny w dwóch wymiarach. W nim wyświetlona zostaje treść zadania do wykonania. Poniżej znajduje się pole tekstowe, które służy do wprowadzania tekstu wyświetlającego się powyżej. Po prawej stronie znajduje się drugi obszar tekstowy, który to wyświetla różne informacje od serwera lub klienta, czyli pełni funkcję dziennika zdarzeń. Poniżej rejestru zdarzeń jest pole tekstowe oznaczone etykietą “Serwer (host:port)”, gdzie należy podać ip i port serwera, z którym ma nastąpić połączenie. Zaraz obok znajdują się dwie etykiety. Pierwsza zawiera informację o wersji aplikacji, natomiast druga o stanie połączenia. W prawym dolnym rogu są 4 przyciski:
...
* Przycisk “Jak grać?” - jest nieaktywny i pełni rolę estetyczną.
Ostatnim obszarem jest, znajdująca się na górze aplikacji lista użytkowników. Każdy element listy (użytkownik) zawiera (od lewej):
...
* ClientGUI - Główna klasa klienta. Deklaruje wszystkie parametry GUI oraz tworzy graficzny interfejs aplikacji klienta. ClientGUI posiada metody, które zmieniają ustawienia komponentów w tej klasie. Zmiany te to:
